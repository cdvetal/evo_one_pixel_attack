...
def crossover_art(p1, p2, alpha=0.5):
  genc1 = []
  genc2 = []
  genp1 = p1['genotype']
  genp2 = p2['genotype']

  for i in range(0, len(genp1)):
    genc1[i] = alpha*genp1[i] + (1-alpha)*genp2[i]
    genc2[i] = alpha*genp2[i] + (1-alpha)*genp1[i]

  c1 = {'genotype': genc1, 'fitness': None, 'confidence': None, 'success': None}
  c2 = {'genotype': genc2, 'fitness': None, 'confidence': None, 'success': None}
  return c1
...
def mutate_por_gene_arithmetic(individual, mutation_rate, w, h, mutation_step_w, mutation_step_h, mutation_step_rgb):
    ...
    for i in range(5):
      ...
      if random.random() < mutation_rate:
        if i == 0:
          new_gene = gene + random.randint(-mutation_step_w, mutation_step_w)
          if new_gene < 0:
             new_gene = 0
          if new_gene > w:
             new_gene = w
        elif i == 1:
          new_gene = gene + random.randint(-mutation_step_h, mutation_step_h)
          if new_gene < 0:
             new_gene = 0
          if new_gene > h:
             new_gene = h
        else:
          new_gene = gene + random.randint(-mutation_step_rgb, mutation_step_rgb)
          if new_gene < 0:
             new_gene = 0
          if new_gene > 255:
             new_gene = 255
        mutated_individual['genotype'][i] = new_gene
    ...
    for it in range(it, NUMBER_OF_ITERATIONS):
      ...
      ## avaliar se é adversarial depois da avaliaçao
        ...
        while len(new_population) < POPULATION_SIZE:
            ...
            # ni = mutate_por_gene_arithmetic(ni, PROB_MUTATION, w, h, mutation_step_w, mutation_step_h, mutation_step_rgb)
            # evaluate([ni], image, true_class, model)
