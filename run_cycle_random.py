...
# Helper functions
from random import with_random
import helper
import gc
...
for model in models:
    ...
    # Create de folder
    ra_folder = f'./results/{modelName}/ra'
    if not os.path.exists(ra_folder):
        os.makedirs(ra_folder)

    # Time
    time_file = f'{ra_folder}/time.csv'
    ...
    file_metrics = f'{ra_folder}/metrics.csv'
    f_metrics = open(file_metrics, 'w')
    writer_metrics = csv.writer(f_metrics)
    writer_metrics.writerow(['run', 'success rate', 'adv prob label', 'time'])
    
    # Write csv with pixels covered
    file_cover = f'{ra_folder}/covered_pixels.csv'
    ...
    for run in range(1, nruns+1):
        ...
        run_folder = f'{ra_folder}/run_{run}'
        ...
        for i in range(n_samples):
            ...
            if not os.path.exists(img_folder):
                os.makedirs(img_folder)
                                                        #with_random(img, true_class, model, pop_size, generations, bounds, folder_path, SEED):
            best_ind, suc, suc_act_total, n_covered_pixels = with_random(img, label, model, pop_size, generations, bounds, img_folder, seeds[run-1])
            ...
            writer_npixels.writerow(['ra', run, img_idx, i, suc])
            f_npixels.close()
            ...
            # Save img metrics
            folder_metrics_img = f'{ra_folder}/metrics_img'
   ...
   # Write mean metrics
    ...
    writer_means.writerow(['ra', mean_suc_rate_dataset])
    f_means.close()
    ...
    csv_file = f'{ra_folder}/parameters.csv'

    # Create a list of parameter names and their corresponding values
    parameter_names = ["nruns", "seeds", "n_samples", "samples", "n_evaluations", "pixels"]
    parameter_values = [nruns, seeds, n_samples, samples, pop_size*generations, pixels]
    ...
    for img_idx, success_rate in dict_success_rate.items():
        ...
        writer_metrics_img.writerow([counter, img_idx, 'ra', mean_success_rate, [mean_adv_prob1, mean_adv_prob2]])
        counter += 1
    f_metrics_img.close()